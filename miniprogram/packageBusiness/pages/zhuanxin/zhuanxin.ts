...
Page({
  ...
  goToAuth() {
    wx.navigateTo({
      url: `/pages/auth/auth?return=${encodeURIComponent('/packageBusiness/pages/zhuanxin/zhuanxin')}`
    })
  },
  ...
  chooseImageFile() {
    wx.chooseMedia({
      ...
      success: (res) => {
        ...
        // 使用统一的文件验证
        const validation = validateFile(`详版财务报告截图.${file.tempFilePath.split('.').pop()}`, file.size, 'detail')
        if (!validation.valid) {
        ...
        this.setData({
          selectedFile: {
            name: `详版财务报告截图.${file.tempFilePath.split('.').pop()}`,
            size: file.size,
  ...
  async onStartAnalysis() {
    ...
    try {
      ...
      if (result.result && (result.result as any).success) {
        ...
        throw new Error((result.result as any)?.error || '创建分析任务失败')
      }
    ...
    try {
      ...
      if (result.result && (result.result as any).success) {
        ...
        const reports = (result.result as any).data.reports.map((report: any) => ({
          id: report.reportId,
          title: `深度财务分析报告 - ${report.fileName}`,
  ...
  goHome() {
    ...
    if (this.data.generating) {
      wx.showModal({
        title: '提示',
        content: '专业版财务分析报告正在生成中，离开页面不会中断处理，您可以稍后回来查看结果',
        confirmText: '继续离开',
        cancelText: '留在此页',
        success: (res) => {
          if (res.confirm) {
