...
Page({
  ...
  async startAnalysis() {
    ...
    try {
      // 调用模拟API生成报告
      const result = await mockApi.generateReport('detail', selectedFile)
  ...
  async loadReportList() {
    ...
    try {
      const result = await mockApi.getReportList('detail')
      
      if (result.success) {
        // 过滤出详版征信报告
        const filteredReports = (result.data || []).filter(report => report.type === 'detail')
        this.setData({
  ...
  onViewReport(e: any) {
    ...
    wx.navigateTo({
      url: `/pages/report/report?type=zhuanxin&title=${encodeURIComponent(report.title)}&date=${report.date}&id=${report.id}`
    })
  },
